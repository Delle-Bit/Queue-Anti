...
## Corpus Check
- 55 files · ~65,865 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 583 nodes · 769 edges · 47 communities (34 shown, 13 thin omitted)
...
4. `pool` - 10 edges
5. `/graphify` - 10 edges
6. `startSpeechRecognition()` - 9 edges
7. `loadLabQueue()` - 8 edges
8. `setVaState()` - 8 edges
9. `vaSay()` - 8 edges
...
- `loadAssistantContext()` --calls--> `buildCustomerStatus()`  [EXTRACTED]
  routes/assistant.js → routes/queue.js
...
Cohesion: 0.09
Nodes (32): APPT_SLOTS, apptNextStep(), apptPrevStep(), bookAppointment(), calendarDate, cancelQueue(), changeCalendarMonth(), checkMandatoryMedicalForm() (+24 more)
...
Cohesion: 0.08
Nodes (24): nextTicketNumber(), adminRoutes, app, assistantRoutes, authRoutes, bcrypt, bodyParser, cors (+16 more)
...
Cohesion: 0.22
Nodes (13): peekTicketNumber(), loadAssistantContext(), buildCustomerStatus(), buildPackagePreview(), express, getCurrentProcessing(), getPackageSteps(), getQueueType() (+5 more)

### Community 13 - "va.js"
Cohesion: 0.20
Nodes (24): addVaHistory(), answerQueueStatus(), answerWaitTime(), bindVaListeners(), clearVaHistory(), dismissVaBubble(), executeVaAction(), openVaHistory() (+16 more)

### Community 14 - "ai_services.js"
Cohesion: 0.06
Nodes (22): aiServices, axios, callMockAI(), checkAIToggle(), dotenv, fs, logAI(), nvidiaFallback() (+14 more)
...
## Knowledge Gaps
- **223 isolated node(s):** `axios`, `{ pool }`, `{ spawn }`, `path`, `fs` (+218 more)
...
- **Why does `pool` connect `database.js` to `server.js`, `queue.js`, `ai_services.js`, `admin.js`, `queue_automation.js`, `packages.js`, `assistant.js`?**
